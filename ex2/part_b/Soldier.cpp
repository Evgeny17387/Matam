...
namespace mtm
{
    ...
    Soldier::Soldier(Team team, units_t health, units_t ammo, units_t range,
        units_t power):
        Character(team, health, MOVE_RANGE, RELOAD_AMMO, AMMO_PER_ATTACK,
    ...
    bool Soldier::isOnTheSameLine(const GridPoint& coordinates_src,
        const GridPoint& coordinates_dst)
    {
        if ((coordinates_src.col != coordinates_dst.col) &&
            (coordinates_src.row != coordinates_dst.row))
        {
            return false;
        }
    ...
    bool Soldier::canAttack(Character* character,
    ...
    void Soldier::singleAttack(Character* defender,
        const GridPoint& coordinates_dst, const GridPoint& attack_coordinates)
    {
        units_t impact = 0;
        ...
        else
        {
            ...
            {
                int attack_range =
                    GridPoint::distance(coordinates_dst, attack_coordinates);
        ...
        const GridPoint& coordinates_dst)
    {
        ...
        for (int row_diff = -this->max_range_affect;
            row_diff <= this->max_range_affect; row_diff++)
        {
            for (int col_diff = -this->max_range_affect;
                col_diff <= this->max_range_affect; col_diff++)
            {
                ...
                shared_ptr<Character> defender =
                    board[attack_coordinates.col][attack_coordinates.row];
                ...
                singleAttack(defender.get(), coordinates_dst,
                    attack_coordinates);
            }
        }
    }
