...
namespace mtm
{
    ...
    ExamDetails::ExamDetails(int course_id, int month, int day, double time, int duration, const string link):
        course_id(course_id), month(month), day(day), time(time), duration(duration), link(link)
    {
    ...
    bool ExamDetails::operator<(const ExamDetails& examDetails) const
    {
        ...
        {
            return this->time < examDetails.time;
        }
    }
    ...
    ostream& operator<<(ostream& os, const ExamDetails& examDetails)
    {
        ...
        os << "Time: " << examDetails.day << "." << examDetails.month << " at " << hours << ":" << std::setw(2) <<
    ...
    {
        return ExamDetails(MATAM_COURSE_ID, MATAM_EXAM_DATE_MONTH, MATAM_EXAM_DATE_DAY, MATAM_EXAM_TIME,
